...
public class SubsidyAllocationServiceImpl implements SubsidyAllocationService {
    ...
    private final SubsidyAllocationRepository allocationRepository;
    private final SubsidyBatchRepository batchRepository;
    private final CollegeRepository collegeRepository;
    private final SubsidyApplyRepository applyRepository;

    public SubsidyAllocationServiceImpl(SubsidyAllocationRepository allocationRepository,
                                        SubsidyBatchRepository batchRepository,
                                        CollegeRepository collegeRepository,
                                        SubsidyApplyRepository applyRepository) {
    ...
    @Override
    @Transactional
    public void allocateQuota(AllocationCreateRequest request, CurrentUser currentUser) {
        AllocationAccess access = resolveAccess(currentUser);
        Integer currentUserRole = access.allocatorRole();
        Long currentUserCollegeId = access.collegeId();
        // 1. 角色权限校验 + 预算控费
    ...
    @Override
    public AllocationSummaryResponse getSummary(Long batchId, CurrentUser currentUser) {
        AllocationAccess access = resolveAccess(currentUser);
        Integer currentUserRole = access.allocatorRole();
        Long currentUserCollegeId = access.collegeId();
        BigDecimal total;
    ...
    private AllocationAccess resolveAccess(CurrentUser currentUser) {
        if (currentUser.userType() == 4) {
            return new AllocationAccess(1, null);
        }
        if (currentUser.userType() == 3) {
            Long collegeId = applyRepository.findCollegeIdByUserId(currentUser.id())
                    .orElseThrow(() -> new BusinessException(40400, "未找到学院归属"));
            return new AllocationAccess(2, collegeId);
        }
        throw new BusinessException(40300, "仅学校资助中心或学院管理员可以管理补助额度");
    }

    private record AllocationAccess(int allocatorRole, Long collegeId) {
    }
}
