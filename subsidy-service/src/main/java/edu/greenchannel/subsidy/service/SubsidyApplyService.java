...
public class SubsidyApplyService {
    ...
    public PageResult<SubsidyApplyView> listApplies(CurrentUser user, Long batchId, Integer status,
                                                   String studentName, Long collegeId, int page, int size) {
        return switch (user.userType()) {
            ...
            case 2, 3 -> {
                ...
                yield repository.searchForCollege(userCollegeId, batchId, status, studentName, page, size);
            }
            case 4 -> repository.searchForSchool(batchId, status, studentName, collegeId, page, size);
