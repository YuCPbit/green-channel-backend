...
> 状态：现行数据库设计说明
> 最后核验：2026-07-24（A 接管功能和 52 张表均已与建库脚本核对）
...
9. **资助方案、申诉与反馈组**：管理方案生命周期、申诉窗口和节点处理、满意度问卷及答卷汇总。
10. **勤工助学组**：管理批次、岗位、报名、录用、岗位变动、考勤、评价、薪酬和协议。
...
erDiagram
    ...
    gc_work_study_hire ||--o{ gc_work_study_movement : "申请调岗或离岗"
    gc_work_study_position ||--o{ gc_work_study_movement : "原岗位/目标岗位"
...
### 7.2.1 资助方案、申诉与满意度 ER 图

```mermaid
erDiagram
    gc_student ||--o{ gc_appeal : "提交"
    gc_subsidy_apply ||--o| gc_appeal : "被申诉"
    gc_gift_pack_apply ||--o| gc_appeal : "被申诉"
    gc_satisfaction_survey ||--o{ gc_satisfaction_response : "包含答卷"
    gc_student ||--o{ gc_satisfaction_response : "填写"
```
...
#### `gc_work_study_movement` 岗位变动申请表
| 字段名 | 类型 | 必填 | 说明 |
| :--- | :--- | :--- | :--- |
| `hire_id` | BIGINT | 是 | 原在岗记录ID |
| `from_position_id` / `to_position_id` | BIGINT | 是/否 | 原岗位和调岗目标岗位 |
| `movement_type` | VARCHAR(20) | 是 | TRANSFER-调岗 / LEAVE-离岗 |
| `status` | TINYINT | 是 | 1-待审批 2-已通过 3-已驳回 |
| `reviewer_id` / `review_comment` | BIGINT/VARCHAR | 否 | 审批人和审批意见 |
...
| 表 | 核心字段 | 状态/约束 |
| :--- | :--- | :--- |
| `gc_aid_plan` | 资金来源、金额模式、名额、期限、条件表达式 | 0-草稿 1-发布 2-下线 |
| `gc_appeal` | 原业务/申请、学生、申诉理由、附件、处理节点、结论 | 同一学生对同一原申请只能有一条申诉 |
| `gc_satisfaction_survey` | 目标业务/批次、开放日期 | 0-草稿 1-发布 2-结束 |
| `gc_satisfaction_response` | 问卷、学生、1—5 分、建议 | 每名学生每份问卷只能提交一次 |
